...
const SLOT_BY_WEEKDAY: Record<number, Slot[]> = {
  ...
  3: [
    ...
    { label: '02:00 PM', available: true },
    { label: '03:30 PM', available: false },
  ...
  4: [
    ...
    { label: '06:30 PM', available: true },
    { label: '07:30 PM', available: false },
  ],
  5: [
    { label: '08:30 AM', available: true },
    { label: '09:30 AM', available: true },
...
const DirectSiteVisit: React.FC<Props> = ({ onBack, showBack = true }) => {
  ...
  const availableSlots = useMemo(() => {
    ...
    return SLOT_BY_WEEKDAY[selectedDate.getDay()] ?? [];
  }, [selectedDate]);
    ...
    <div className="flex flex-col gap-3 animate-fade-in">
      {showBack && (
        <div className="flex items-center gap-2">
          <button onClick={onBack} aria-label="Go back" className="p-1.5 hover:bg-[#FFF1E3] rounded-[5px] transition-all duration-300 shadow-sm hover:shadow-md">
          ...
          <span className="text-[15px] font-extrabold text-[#322822]">Direct Site Visit</span>
        </div>
      )}
      ...
      <div className="space-y-2.5">
        {/* ── Date Picker ── */}
        <div>
          <p className="text-[11px] font-bold text-[#322822] tracking-wide mb-1">Preferred Date</p>
          <Popover>
        ...
        <button className="w-full bg-gradient-to-r from-[#F06A00] to-[#D95700] text-white py-2.5 rounded-[5px] font-extrabold text-[13px] shadow-xl shadow-orange-500/30 hover:shadow-2xl hover:shadow-orange-500/40 transition-all duration-300 hover:scale-102">
          Confirm Schedule
        </button>
      </div>
    </div>
